import type { base64, imageURL } from '../../types/@ae_utbm/core';
...
if (!ArrayBuffer.prototype.toBase64) {
  ArrayBuffer.prototype.toBase64 = function toBase64(
    type = 'image',
    extension = 'webp'
  ): base64 {
    let binary = '';
...
if (!ArrayBuffer.prototype.toImageURL) {
  ArrayBuffer.prototype.toImageURL = function toImageURL(
    type = 'image',
    extension = 'webp'
  ): imageURL {
    const bytes = new Uint8Array(this);
    ...
    try {
      ...
      return JSON.parse(
        String.fromCharCode.apply(
          null,
          new Uint8Array(this) as unknown as number[]
        )
      );
    } catch (error) {
      if (error instanceof SyntaxError)
        throw new SyntaxError('The ArrayBuffer cannot be converted to JSON');
      throw error;
    }
  };
}
